...
class GithubService {
  ...
  static Future<String?> recupererVersion() async {
    ...
    final contenuDecode = utf8.decode(
      base64Decode(content.replaceAll('\n', '')),
    );

    final json = jsonDecode(contenuDecode) as Map<String, dynamic>;

    return json['version'] as String?;
  }


  Future<String?> demanderTokenGitHub(BuildContext context) {
  ...
  static Future<void> envoyerPlanning({
    ...
    if (tokenNettoye.isEmpty) {
      throw Exception('Le token GitHub est vide.');
    }

    if (version.trim().isEmpty) {
      throw Exception('La version du planning est vide.');
    }

    if (planningBase64.trim().isEmpty) {
      throw Exception('Le planning compressé est vide.');
    }
    ...
    final body = <String, dynamic>{
      'event_type': 'import_planning',
      'client_payload': {'version': version, 'planning': planningBase64},
    };
    ...
    try {
      ...
      throw Exception('Impossible de contacter GitHub : $e');
    }
    ...
    switch (response.statusCode) {
      ...
      case 404:
        message = 'Repository GitHub introuvable ou inaccessible.';
        break;
      ...
      default:
        message = 'Erreur GitHub (${response.statusCode}).';
    }
